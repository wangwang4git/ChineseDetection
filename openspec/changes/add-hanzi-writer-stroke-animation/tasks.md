...
- [ ] 1.3 使用 hanzi-writer-view 原生组件渲染汉字
- [ ] 1.4 定义 `animateStroke()` 方法，触发笔画动画
- [ ] 1.5 暴露组件方法供父组件调用：`defineExpose({ animateStroke })`
...
- [ ] 2.5 实现 `handleStrokeTap()` 方法，调用 `mainHanziGrid.animateStroke()`
- [ ] 2.6 添加笔画按钮样式（.stroke-btn）
...
- [ ] 4.1 小程序模拟器：验证汉字正确渲染
- [ ] 4.2 小程序真机：验证笔画动画播放正常（Path2D 兼容性修复后）
- [ ] 4.3 验证汉字切换时 hanzi-writer 实例正确更新
